...
painel.Size = UDim2.new(0, 240, 0, 220)
painel.Position = UDim2.new(0, 180, 0.5, -110)
...
-- Função para detectar a base onde o jogador está
local function detectarBaseAtual()
    for _, model in ipairs(workspace:GetChildren()) do
        if model:IsA("Model") and model:FindFirstChild("Sua Base") then
            if (player.Character and player.Character:FindFirstChild("HumanoidRootPart")) then
                local dist = (model.PrimaryPart or model:FindFirstChild("HumanoidRootPart") or model:FindFirstChildWhichIsA("BasePart", true)).Position - player.Character.HumanoidRootPart.Position
                if dist.Magnitude < 60 then
                    return model:FindFirstChild("Sua Base")
                end
            end
        end
    end
...
-- Teleporte para cima da base
criarBotao("⬆️ Subir", Color3.fromRGB(0, 170, 255), function()
    local base = detectarBaseAtual()
    if base then
        local cframe = base:GetBoundingBox()
        local altura = cframe.Position.Y + (cframe.Size.Y / 2) + 10
        local novaPos = Vector3.new(cframe.Position.X, altura, cframe.Position.Z)
...
RunService.RenderStepped:Connect(function()
    if speedAtivo and player.Character then
...
RunService.RenderStepped:Connect(function()
    if puloAtivo and player.Character then
        local hum = player.Character:FindFirstChildOfClass("Humanoid")
        if hum then
