...
local function atualizarBotaoSubir()
	...
	botaoSubir.MouseButton1Click:Connect(function()
		...
		if not plataformaAtiva then
			...
			plataforma.Size = Vector3.new(40, 1, 40)
			...
			wait(0.1)
			character:MoveTo(plataforma.Position + Vector3.new(0, 4, 0))
			plataformaAtiva = true
		else
			local plataforma = workspace:FindFirstChild("PlataformaAerea")
			if plataforma then
				plataforma:Destroy()
			end
